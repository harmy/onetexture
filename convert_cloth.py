...
def main():    
    for dir in os.listdir(os.path.join(RES_PATH, "衣服")):
        shape, sex = dir.split("_")
        shape = int(shape)
        sex = ["男", "女"].index(sex[:2])
        for key, value in ACTIONS.iteritems():
            ...
            if not os.path.exists(dest_dir):
                os.mkdir(dest_dir)
            dest_filename = os.path.join(dest_dir, "00%04d%s.png" % (shape * 2 + sex, key))
            if os.path.exists(dest_filename):
                print "%s已存在, 跳过该文件." % dest_filename
                continue
            src_dir = os.path.join(RES_PATH, "衣服", dir, value)
            if not os.path.exists(src_dir):
                continue
            print "正在转换: %s ..." % dest_filename
            tmp_dir = os.path.join(TMP_PATH, "衣服", dir, value)
