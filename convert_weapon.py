...
def main():
    for dir in os.listdir(os.path.join(RES_PATH, "武器")):
        shape, itemname = dir.split("_")
        shape = int(shape)
        for key, value in ACTIONS.iteritems():
            dest_dir = os.path.join(OUTPUT_PATH, dir)
            if not os.path.exists(dest_dir):
                os.mkdir(dest_dir)
            dest_filename = os.path.join(dest_dir, "03%04d%s.png" % (shape, key))
            if os.path.exists(dest_filename):
                print "%s已存在, 跳过该文件." % dest_filename
                continue
            src_dir = os.path.join(RES_PATH, "武器", dir, value)
            if not os.path.exists(src_dir):
                continue
            print "正在转换: %s ..." % dest_filename
            tmp_dir = os.path.join(TMP_PATH, "武器", dir, value)
